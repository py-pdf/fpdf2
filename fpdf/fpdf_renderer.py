...
class RendererTemplate(RendererBase):
    ...
    def draw_path(self, gc, path, transform, rgbFace=None):
        ...
        with p.rect_clip(clip_x0,clip_y0,clip_x1-clip_x0,clip_y1-clip_y0)  if clip_rect is not None else nullcontext() as clip:

            with p.local_context(stroke_opacity=stroke_opacity, fill_opacity=fill_opacity, line_width=mm_line_width):
                #p.set_draw_color(rgb[0]*255, rgb[1]*255, rgb[2]*255)
    ...
    def draw_text(self, gc, x, y, s, prop, angle, ismath=False, mtext=None):
        ...
        th = self._fpdf.font_size_pt * PT_TO_MM * self._font_scaling  # Default text height in mm
        ...
        # We're expecting a FontProperties instance
            ...
            self._fpdf.set_font(prop.get_name(), size=prop.get_size() * self._font_scaling)
            ...
            rotated_bbox = Affine2D().rotate_deg(angle).transform(((0,0),(tw,0),(tw,th),(0,th)))
        ...
        color = gc.get_rgb()
        self._fpdf.set_text_color(int(color[0]*255), int(color[1]*255), int(color[2]*255))
        #print("Color:", color)
...
class FigureCanvasTemplate(FigureCanvasBase):
    ...
    def draw(self):
        ...
        renderer = RendererTemplate(self.figure, self.figure.dpi, self._fpdf, self._scale, self._trans, width, height)
