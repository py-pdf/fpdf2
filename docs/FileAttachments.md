...
Pass `hide_icon=True` to suppress that icon while keeping the file embedded and reachable:

```python
pdf = FPDF()
pdf.add_page()
pdf.file_attachment_annotation(__file__, x=50, y=50, hide_icon=True)
pdf.output("file_attachment_annotation.pdf")
```
